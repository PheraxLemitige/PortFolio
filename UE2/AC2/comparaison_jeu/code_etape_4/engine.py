...
def squareCollision(q1, q2):
    '''
    Determines if two squares intersect or not
    '''
    #test if square 1 is too much to the right of the square 2
    if q2[0] >= q1[0] + constants.PERSON_RADIUS * 2:
        #test if square 1 is too much to the left of the square 2
        if q2[0] + constants.PERSON_RADIUS * 2 <= q2[0]:
            #test if square 1 is too much to the down of the square 2
            if q2[1] >= q1[1] + constants.PERSON_RADIUS * 2:
                #test if square 1 is too much to the up of the square 2
                if q2[1] + constants.PERSON_RADIUS * 2 <= q1[1]:
                    return False
    return True
...
def circleCollision(c1, c2):
    ...
    '''
    return math.sqrt((c2[0] - c1[0]) ** 2 + (c2[1] - c1[1]) ** 2) - constants.PERSON_RADIUS <= 0
...
def computeCollisions(persons, personNumber):
    ...
    for j in range (len(persons)):
        ...
        if ( j != personNumber):
            ...
            if squareCollision((p[0] - constants.PERSON_RADIUS, p[1] - constants.PERSON_RADIUS),(q[0] - constants.PERSON_RADIUS,q[1] - constants.PERSON_RADIUS)):
