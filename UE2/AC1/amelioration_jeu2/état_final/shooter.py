...
def mechant(targets, sens_x, sens_y, vitesse):
    for target in targets:
        target['rectangle'][0] = target['rectangle'][0] + constants.TARGET_SPEED * sens_x * vitesse 
        target['rectangle'][1] = target['rectangle'][1] + constants.TARGET_SPEED * sens_y * vitesse
    return targets
...
def manage_events(world, targets) -> bool:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and world['energie'] > 0:
            if world['touche_bouge'] == event.key and world['temps'] < 6 :
                world['temps'] = world['temps'] + 0.01
                world['energie'] = world['energie'] - 2
            elif world['touche_bouge'] != event.key:
                world['temps'] = 0
            if event.key == pygame.K_s:
                targets = mechant(targets, 0, 1, world['temps'])
            elif event.key == pygame.K_z:
                targets = mechant(targets, 0, -1, world['temps'])
            elif event.key == pygame.K_d:
                targets = mechant(targets, 1, 0, world['temps'])
            elif event.key == pygame.K_q:
                targets = mechant(targets, -1, 0, world['temps'])
            world['touche_bouge'] = event.key
        else:
            if world['energie'] < 200:
                if world['energie_attend'] == 20:
                    world['energie'] = world['energie'] + 2
                else:
                    world['energie_attend'] += 1
